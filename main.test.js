...
describe("/posts/:postId", function () {
 ...
 it("should delete the post with the given ID", function (done) {
    ...
    mainHandler.deletePosts(res,req);
  }); 
}); */
...
describe('signup', () => {
  it('should return an error when the user submits blank fields', async () => {
    const response = await request(app)
      .post('/registered')
      .send({ firstname:'',lastname:'',username: '',email:'', plainTextPassword: '' })
      .set('Accept', 'application/json');
...
describe('signup', () => {
  it('should return an error when the user submits a password that is too easy', async () => {
    const response = await request(app)
      .post('/registered')
      .send({ firstname:'John',lastname:'Doe',username: 'jdoe123',email:'jdoe@gold.ac.uk', plainTextPassword: '0' })
      .set('Accept', 'application/json');
...
describe('signup', () => {
  it('should return an error when the user submits wrong email', async () => {
    const response = await request(app)
      .post('/registered')
      .send({ firstname:'jane',lastname:'smith',username: 'jsmith',email:'jsmith', plainTextPassword: 'Password1' })
      .set('Accept', 'application/json');
